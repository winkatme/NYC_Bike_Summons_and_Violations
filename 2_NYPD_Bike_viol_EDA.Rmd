...
ts_fc <- ts_daily_snaive_mod |> forecast(h = "2 years")
...
Takeaway: Plot does not tell us much.  As error bars increase, there can be as many as 80000+ cyclists in a day, or as few as in the negative numbers.  

### Seasnal naive forecast on monthly totals
...
str(ts_monthly_snaive_mod)

```

Takeaway:  This looks a lot better.  Let's just analyze monthly
...
```{r, warning=FALSE}
# mean(augment(ts_monthly_snaive_mod)$.resid, na.rm=TRUE)  # 74750.9
...
```{r}
#ljung-box test to see if residuals differ from white noise:
...
Takeaways:  Residuals look normally distributed enough and uncorrelated enough.  Sig lag at 12 months, but that's to be expected as there's yearly seasonality. Also sig at 5 months.  Will safely ignore this. One potential issue is that the mean is not quite 0, it's 74750.9.  
...
Takeaway:  Looks good.  Pretty accuarate.  Let's compare to other methods.
