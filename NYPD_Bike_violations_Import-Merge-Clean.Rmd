...
# 92 for city_nm to be removed/cleaned

# 2068 for description to be filled in.
```
...
```{r}
# change all 4 violation codes to chosen values
...
Since these 16 have no violation code or description, I think we can safely delete them, as they provide no useful information.

```{r}
df_bike_violations <- df_bike_violations |>
  filter(!is.na(description))

# are there any na's left for description or violation code?
df_bike_violations |>
  filter(is.na(description) | is.na(violation_code))
...
By changing the city_nm == 'New York' values  to their specific boroughs, we also manage to remove the NAs.
...
Takeaway:  these dont have longitudinal or latitudinal info.  We may therefore remove them
...
Takeaway: none of the 92 have longitude or latitiude info either.  Can be removed.
