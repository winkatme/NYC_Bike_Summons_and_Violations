...
Takeaway 1: there are 103 'na' entries for location coordinate data.  We can ignore these for now, and remove them when data location is required.  There is no sense in discarding the rest of the information, like violation codes, which can be used.

Takeaway 2: 92 for city_nm to be removed/cleaned

Takeaway 3: 2068 for description to be filled in.
...
Are there any NA's remaining for 'description'?
...
By first changing the city_nm == 'New York' values to their specific boroughs, we also manage to fill-in/remove the NAs.

Let's view the New York city_nm values on a map to see if there are any outside of Manhattan
...
# check again for New York values
df_bike_violations |>
  group_by(city_nm) |>
  summarize(sum = sum(n()))
```

Takeaway: better - now only 3 NEW YORK and 92 na.
...
Takeaway:  these dont have longitudinal or latitudinal info.  We may therefore remove them.  And since filter in dplyr automatically removes NAs, the rest of the NA's are removed as well.  So that is 92 NA's + 3 city_nm=='New York' = 95 removed values.  We are left with 122,509 rows, when we started with 122,604.
