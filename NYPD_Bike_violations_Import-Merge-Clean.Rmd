...
### To Do: look up duplicate counters.  SO far found 2 exact counters by manhattan side of manhattan bridge

```{r}
# first, find counters via latitudes with more than one unique value
...
# 1. willis ave has 3 
# according to metadata file (DataDictionary-BicycleCounters.xlsx on web page with original dataset), the willis ave station counts bikes (300029647), pedestrians (300029648) and has the sume of those (300028963).  Therefore, let's remove the two that are not exclusively bikes.

df_bicycle_counters_boroughs |> 
  filter(!id==300028963 & !id==300029648)
...
# 3.  manhattan bridge has 2 +2+2 for 'interference' .
# let's look at all man bridge.
man_br_ids<-df_bicycle_counters_boroughs |> 
  filter(grepl("Manhattan Bridge", name)) |> 
  pull(id)
...
# double-check min and max dates
df_counts_raw |> 
  mutate(date = mdy_hms(date)) |> 
  filter(id==100047029) |> 
  slice(which.min(date),which.max(date))
# From metadata file:  'As of May 7, 2019 the Display Manhattan Side counter is the primary source of bike counts on the bike path of the bridge'. since the range is 2012-2023, let's choose just the Manhattan Bridge Display Bike Couinter 100047029 for all man bridge data.


# filter out all duplicate/unnecessary counters (includes 'Test' 300020692):
df_bicycle_counters_boroughs |> 
  filter(!id==300028963 & !id==300029648 &!id==100009426 & !id==100055175 & !id==100009429 & !id==100005020 & !id==100051865 & !id==100048744 & !id==100062893 & !id==300020692)


# note there are some brooklyn bridge dupes, too:  
df_counts_raw |> 
  mutate(date = mdy_hms(date)) |> 
