...
df_bike_violations <- readr::read_csv("df_bike_violations_2023-07-18.csv")
...
## Create sf for spatial analysis

```{r}
# attempt to create st

sf_bike_violations <- readr::read_csv("df_bike_violations_2023-07-18.csv") |> 
  filter(!is.na(longitude)) |> 
  st_as_sf(coords=c("longitude","latitude"), crs=4326) # remember x=lon and y=lat

class(sf_bike_violations)
...
## Create monthly and yearly total data tsibble
...
# looks like max per year is 14.  so not all are used at once.
```
...
## Which bike counters counted the most cyclists in 2023?
...
# top 3 are the same.  4 and 5 are switched
```
...
# ans: 184? 183?  review na deletions
```
