...
Contains all NYPD violation records from Jan 1, 2023 to Mar 30, 2023.
...
Takeaway 1: there are 103 'na' entries for location coordinate data.  We can ignore these for now, and remove them when data location is required.  There is no sense in discarding useful information, like violation codes and violation dates.
...
We then remove any unchanged city_nm == 'New York' rows.  Because we use dplyr::filter(), this has the effect of also removing any NAs.
