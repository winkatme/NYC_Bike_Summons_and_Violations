...
test<-ts_monthly_total |> 
  select(!monthly_total_violations) |> 
  filter_index("2022-06"~.)

range(train$date)
range(test$date)

range(test_daily$date)

train_daily<-ts_daily_total |> 
  rename(date=violation_date) |> 
  select(!c(countid, id, status)) |> 
  filter_index(~"2022-05")

test_daily<-ts_daily_total |> 
  rename(date=violation_date) |> 
  select(!c(countid, id, status)) |> 
  filter_index("2022-06"~.)
```
...
ts_daily_snaive_mod <- ts_daily_total |> 
  model(model = NAIVE(daily_total_cyclists ~ lag(12)))

ts_fc <- ts_daily_snaive_mod|> forecast(h = "1 month")
...
# ETS_AAdA (best model) evaluation

```{r}
mod_ETS_AAdA <- train %>%
  model(ETS(monthly_total_cyclists ~ error("A") + trend("Ad") + season("A")))

gg_tsresiduals(mod_ETS_AAdA)
```

Takeaway: resids look normal enough.  possible autocorrelation. Tests:

```{r}
augment(mod_ETS_AAdA) |> features(.innov, box_pierce, lag = 12)
augment(mod_ETS_AAdA) |> features(.innov, ljung_box, lag = 12)
```
...
Takeaway:  This model seems to adequately keep the summer peaks steady while slowly increasing the winter lull.  This fits our eda.  
